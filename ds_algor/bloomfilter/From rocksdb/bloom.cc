...
class FullFilterBitsBuilder : public FilterBitsBuilder {
 public:
  ...
  // dst len is >= 5, 1 for num_probes, 4 for num_lines  //其中 1个字节存 num_probes, 4个字节存 num_lines
  // 根据 CACHE_LINE 来分配空间、计算hash值对应的 位数组的索引（先计算索引在哪个CACHE_LINE上，再得到 在CACHE_LINE内有偏移）
 ...
 private:
  ...
  size_t num_probes_;   //hash函数的个数
  std::vector<uint32_t> hash_entries_;    //存储所有key的 hash值
...
class FullFilterBitsReader : public FilterBitsReader {
 ...
 private:
  ...
  uint32_t data_len_;
  size_t num_probes_;   //hash函数的个数
  uint32_t num_lines_;  //cache line的个数
