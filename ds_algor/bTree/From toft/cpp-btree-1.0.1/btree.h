...
class btree_node {
 public:
  ...
  struct leaf_fields : public base_fields {
	...
	//前面 count 个value才是有效的.
    mutable_value_type values[kNodeValues];
  };

  //分支结点 也存value，说是B树，而不是B+树
...
void btree_node<P>::rebalance_right_to_left(btree_node *src, int to_move) {
  ...
  assert(position() + 1 == src->position());  //bzh: 为什么
  ...
  //将 src节点中的value[0...to_move-1] 移动到 本节点中.
  for (int i = 1; i < to_move; ++i) {
    value_swap(count() + i, src, i - 1);
  }
  // Shift the values in the right node to their correct position.
  //将src节点中的value[0....count-1-to_move] 移动到 src节点的value[to_move...count-1]
...
void btree_iterator<N, R, P>::increment_slow() {
  if (node->leaf()) {
    ...
    while (position == node->count() && !node->is_root()) {
      ...
      position = node->position();   //用本节点在父结点中的position
      node = node->parent();  //向上走
    }

    if (position == node->count()) {  //说明已经到根结点了
      *this = save;
    }
  } else {
	//node 移动为 node的position+1 索引的子树中的 最左孩子
    assert(position < node->count());   //bzh: why???
